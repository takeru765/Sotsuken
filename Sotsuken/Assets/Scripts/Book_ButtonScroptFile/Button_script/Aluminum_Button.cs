...
public class Aluminum_Button : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        Btrash_alumi = TrashPoint.alumi;//引用した数値を入れる変数
  
        if (Btrash_alumi >= 1)//仮の数値。登録したごみが１個以上になった場合に、図鑑の項目が出現するようになる。
        {
           ...
           // gameObject.GetComponent<Image>().color = new Color(255f, 255f, 255f, 255f);//画像の透明度をもとにもどす
        }
